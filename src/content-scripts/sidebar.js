...
async function main() {
  ...
  const promptBar =
    // styles from chatbotui.com (MIT - Mckay Wrigley)
    `
<div id="prompt-bar" class="flex h-full flex-1 flex-col space-y-1 p-2 scrollbar-trigger" style="transition: transform 0.3s ease-in-out; position:fixed; z-index: 1; right:0; width:260px; background-color: #202123">
  ...
  <div class="relative flex items-center">
    <input id="pg-search-sidebar" class="w-full flex-1 rounded-md border border-white/20 px-4 py-3 pr-10 text-[14px] leading-3 text-white" type="text" placeholder='${tr(
      "search_prompts",
      t,
    )}' value="" style="background-color: #202123">
  </div>
      ...
      <div class="flex w-full flex-col gap-1" id="sidebarPrompts">
        ...
        ${prompts
          .map(
            (prompt) => `
            <div class="relative flex items-center">
                <button data-prompt-id="${
                  prompt.id
                }" class="edit-prompts pgbtn flex w-full text-white cursor-pointer items-center gap-3 rounded-lg p-3 text-sm transition-colors duration-200 hover:bg-500/10">
                    ${svg("lightbulb")}
                    <div data-prompt-id2="${
                      prompt.id
                    }" style="font-size: 12.5px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap; max-width: 165px" class="relative max-h-5 flex-1 overflow-hidden text-ellipsis whitespace-nowrap break-all pr-4 text-left text-[12.5px] leading-3">${
                      prompt.title
                    }</div>
                </button>
                <div class="absolute right-1 z-10 flex text-gray-300">
                    <button data-prompt-id3="${
                      prompt.id
                    }" style="min-width: 20px" class="p-1 can text-neutral-400 svg-hover">
                        ${svg("trash")}
                    </button>
                </div>
            </div>`,
...
`;
  ...
  document
    .getElementById("pg-search-sidebar")
  ...
  async function newBlank() {
    ...
    const html = `<div class="relative flex items-center">
                ...
                <div class="absolute right-1 z-10 flex text-gray-300">
                    <button data-prompt-id3="${
                      prompt.id
                    }" style="min-width: 20px" class="pg-trash p-1 text-neutral-400 svg-hover">
                        ${svg("trash")}
                    </button>
                </div>
            </div>`;
  ...
  async function editPrompt(id) {
    ...
    document
      .getElementById("save-prompt")
      .addEventListener("click", () => updatePrompt(id));
  }
  ...
  async function updatePrompt(id) {
    ...
    let title;
    if (promptIndex !== -1) {
      // Ensure a valid index is returned by getObjectIndexByID function
      ...
      prompts[promptIndex].tags = document
        .getElementById("prompt-tags")
        .value.split(",")
        .filter((tag) => tag !== "");
      prompts[promptIndex].category =
      ...
      <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
          ...
          <div style="width: 80%;" class="dark:bg-gray-900 dark:text-gray-200 dark:border-netural-400 inline-block max-h-[400px] transform overflow-hidden rounded-lg border border-gray-300 bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all dark:bg-[#202123] sm:my-8 sm:max-h-[600px] sm:w-full sm:max-w-lg sm:p-6 sm:align-middle" role="dialog">
            <div class="text-sm font-bold text-black dark:text-gray-200">${tr(
              "name",
              t,
            )}</div>
            <input style="border-color: #8e8ea0" id="prompt-name" value="${name}" class="my-2 w-full rounded-lg border border-neutral-500 px-4 py-2 text-neutral-900 shadow focus:outline-none dark:border-neutral-800 dark:border-opacity-50 dark:bg-gray-800 dark:text-neutral-100" placeholder="${tr(
              "prompt_name",
              t,
            )}" value="">
            <div class="my-2 text-sm font-bold text-black dark:text-gray-200">${tr(
              "prompt",
              t,
            )}</div>
            <textarea style="border-color: #8e8ea0" id="prompt-text" class="my-2 w-full rounded-lg border border-neutral-500 px-4 py-2 text-neutral-900 shadow focus:outline-none dark:border-neutral-800 dark:border-opacity-50 dark:bg-gray-800 dark:text-neutral-100" placeholder="${tr(
              "prompt_body",
              t,
            )}" rows="10" style="resize: none;" spellCheck="false">${text}</textarea>
            <div class="text-sm font-bold text-black dark:text-gray-200">${tr(
              "tags",
              t,
            )}</div>
            <input style="border-color: #8e8ea0" id="prompt-tags" value="${tags}" class="mt-2 w-full rounded-lg border border-neutral-500 px-4 py-2 text-neutral-900 shadow focus:outline-none dark:border-neutral-800 dark:border-opacity-50 dark:bg-gray-800 dark:text-neutral-100" placeholder="${tr(
              "prompt_tags",
              t,
            )}" value="">
            <div class="my-2 text-sm font-bold text-black dark:text-gray-200">${tr(
            ...
            <div class="">
              <select id="prompt-category" class="dark:bg-gray-800 border border-neutral-500 text-sm rounded block w-full">
                ...
                <option value="Academic Writing">${tr(
                  "category_academic_writing",
                  t,
                )}</option>
                <option value="Bypass &amp; Personas">${tr(
                  "category_bypass_personas",
                  t,
                )}</option>
                <option value="Education &amp; Learning">${tr(
                  "category_education_learning",
                  t,
                )}</option>
                <option value="Expert/Consultant">${tr(
                  "category_expert_consultant",
                  t,
                )}</option>
                <option value="Fun &amp; Games">${tr(
                  "category_fun_games",
                  t,
                )}</option>
                <option value="Fitness, Nutrition, &amp; Health">${tr(
                  "category_fitness_nutrition_health",
                  t,
                )}</option>
                <option value="Fiction Writing">${tr(
                  "category_fiction_writing",
                  t,
                )}</option>
                <option value="Music">${tr("category_music", t)}</option>
                <option value="Nonfiction Writing">${tr(
                  "category_nonfiction_writing",
                  t,
                )}</option>
                <option value="Other">${tr("category_other", t)}</option>
                <option value="Philosophy &amp; Logic">${tr(
                  "category_philosophy_logic",
                  t,
                )}</option>
                <option value="Poetry">${tr("category_poetry", t)}</option>
                <option value="Programming &amp; Technology">${tr(
                  "category_programming_technology",
                  t,
                )}</option>
                <option value="Speeches &amp; Scripts">${tr(
                  "category_speeches_scripts",
                  t,
                )}</option>
                <option value="Social Media &amp; Blogging">${tr(
                  "category_social_media_blogging",
                  t,
                )}</option>
                <option value="Travel">${tr("category_travel", t)}</option>
                <option value="Therapy &amp; Life-help">${tr(
                  "category_therapy_life_help",
                  t,
                )}</option>
              </select>
            </div>
            ...
            <button id="save-prompt" type="button" class="w-full px-4 py-2 mt-6 border rounded-lg shadow border-neutral-500 text-neutral-900 hover:bg-neutral-100 focus:outline-none dark:border-neutral-800 dark:border-opacity-50 dark:bg-gray-800">${tr(
              "save",
              t,
            )} </button>
          </div>
  ...
  function getSuggestedPrompts(searchTerm) {
    ...
    suggestions.forEach((s) =>
      s.addEventListener("mouseenter", () =>
        focusEl(s.getAttribute("data-idx")),
      ),
    );
    suggestions.forEach((s) =>
      s.addEventListener("mouseup", () =>
        selectPrompt(
          prompts.find(
            (prompt) => prompt.id === s.getAttribute("data-prompt-id4"),
          )?.text,
        ),
      ),
    );
