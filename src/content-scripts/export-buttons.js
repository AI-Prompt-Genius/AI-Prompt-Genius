...
const ExportButtons = (function()
{
	...
	function getChatBubbleText(chatBubble, isHuman) {
		...
		if (isHuman) {
			...
			text = messageToMarkdown(formatCodeBlocks(chatBubble.querySelector(".prose")).outerHTML) // saves as html
		}
		return text;
	}
	...
	function createDropDown() {
		const button_class = 'flex py-3 px-3 items-center gap-3 rounded-md hover:bg-gray-500/10 transition-colors duration-200 text-white cursor-pointer text-sm';
		...
		<a id="shareExport" class="dropbtn ${button_class}">${shareSVG} ${tr("export")}</a>
		</div>
		`
	...
	function createButtons() { // generated by ChatGPT
		createDropDown();
		addDropDownStyle();
		// add listeners
