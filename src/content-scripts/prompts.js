...
(() => {
	...
	Promise.all([
		...
		.then(values => {
			// After loading both groups of prompts, concat them and do some final cleaning.
			let templates = values[1].prompts.concat(values[0]);
			templates.forEach(template => 
				{
					if(!template.placeholder) template.placeholder = "";
					if(!template.prompt) template.prompt = template.text;
				});
            // Save the array of prompt templates to a global variable
            window.prompttemplates = templates
            // Insert the "Prompt Templates" section into the chat interface
	...
	function insertPromptTemplatesSection () {
		...
		${svg`ChatBubble`}
		<h2 class="${css`h2`}">
		<ul id = "templatesContainer" class="${css`ul`}"></ul>
		...
		<div class="${css`column`} items-center">
		  ...
		  <div class="${css`paginationButtonGroup`}">
			<button id="prevPromptTemplatesButton" class="${css`paginationButton`}" style="border-radius: 6px 0 0 6px">Prev</button>
			<button id="nextPromptTemplatesButton" class="${css`paginationButton`} border-0 border-l border-gray-500" style="border-radius: 0 6px 6px 0">Next</button>
		  </div>
		</div>
		</div>
	  `
		...
		wrapper.querySelector("#prevPromptTemplatesButton").onclick = prevPromptTemplatesPage;
		wrapper.querySelector("#nextPromptTemplatesButton").onclick = nextPromptTemplatesPage;
		
		const templatesContainer = wrapper.querySelector("#templatesContainer");
		
		currentTemplates.forEach((template, i) =>
		{
			const button = document.createElement("button");
			const index = i;
			
			button.setAttribute("class", css(`card`));
			button.onclick = ()=>{selectPromptTemplate(start + index)};
