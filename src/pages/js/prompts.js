...
function load_prompts(prompts, search = false, search_term = "", tagList = []) {
  ...
  //console.log(tagList)
  for (let n = prompts.length - 1; n > -1; n--) {
    // load in reverse order
    let template = document
      .querySelector("#prompt_template")
    ...
    tagsInput.addEventListener("keydown", (event) => {
      if (
        event.key === "Enter" &&
        row.querySelectorAll(".autocomplete-active").length === 0
      ) {
        addTag(id, row);
      }
    });
...
Additional information:
        ...
        target.parentElement.classList.contains("select")
      ) {
...
document
  .querySelector("#new_prompt_button")
  .addEventListener("click", new_blank_prompt);
...
function fetch_templates() {
  ...
  fetch(`${host}/csv/prompts.csv`)
    ...
    .then((records) => {
      return records
        .map(([title, prompt, placeholder, tags, category]) => {
...
document
  ...
  .addEventListener("click", () => loadCuratedPrompts(publicTemps));
...
document
  .querySelector("#modal-search-bar")
  .addEventListener("input", searchCuratedPrompts);
document
  .querySelector(".search-bar")
  .addEventListener("input", searchUserPrompts);
...
function autocomplete(inp, arr) {
  // thanks w3Schools: https://www.w3schools.com/howto/howto_js_autocomplete.asp
...
document
  ...
  .addEventListener("change", category_filter);
