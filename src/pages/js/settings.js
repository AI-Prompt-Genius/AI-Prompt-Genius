function update_settings() {
  ...
  let settings =
    // sorry the syntax is weird
    {
      buttons: buttons,
      dont_inject_prompts: dontInject,
      is_plus: isPlus,
      ctrl_save: ctrlSave,
      auto_send: autoSend,
      ...
      home_is_prompts: homePageIsPrompts,
    };
...
function load_settings() {
  let defaults = {
    ...
    auto_send: true,
    auto_delete: false,
    dont_inject_prompts: false,
    ctrl_save: false,
    disable_history: false,
    home_is_prompts: true,
    message:
      'The following is a transcript of a conversation between me and ChatGPT. Use it for context in the rest of the conversation. Be ready to edit and build upon the responses previously given by ChatGPT. Respond "ready!" if you understand the context. Do not respond with anything else. Conversation:\n',
  };
  chrome.storage.local.get({ settings: defaults }, function (result) {
    //console.log(result)
    document.querySelector("#exportButtonsToggle").checked =
      result.settings.buttons ?? true;
    document.querySelector("#autoDeleteToggle").checked =
      result.settings.auto_delete ?? false;
    document.querySelector("#autoSendToggle").checked =
      result.settings.auto_send ?? false;
    document.querySelector("textarea").value = result.settings.message ?? "";
    document.querySelector("#ctrlSave").checked =
      result.settings.ctrl_save ?? false;
    document.querySelector("#default-page").checked =
      result.settings.home_is_prompts ?? true;
    document.querySelector("#disable-history").checked =
      result.settings.disable_history ?? false;
    document.querySelector("#disable-prompt-injection").checked =
      result.settings.dont_inject_prompts ?? false;
    document.querySelector("#chatgpt-plus").checked =
      result.settings.is_plus ?? false;
  });
...
document.body
  .querySelector("#light_dark")
  .addEventListener("click", function f() {
    setTimeout(settings_dark, 0);
  });
