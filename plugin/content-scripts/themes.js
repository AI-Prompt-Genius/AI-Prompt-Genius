...
function changeTheme(theme, onload = false) {
  ...
  function selectAudio(url) {
    ...
    if (!onload) {
      ...
      // this is due to a Chrome autoplay limitation. See: https://developer.chrome.com/blog/autoplay/
      document.body.addEventListener(
        "keydown",
        () => setTimeout(() => themeAudio.play(), 500),
        { once: true },
      );
    }
  }
  ...
  const host =
    "https://raw.githubusercontent.com/benf2004/ChatGPT-Prompt-Genius/master/public";
...
function createThemeSelectButton() {
  ...
  wrapper.setAttribute(
    "class",
    "flex px-3 items-center gap-3 rounded-md hover:bg-gray-500/10 transition-colors duration-200 text-white cursor-pointer text-sm",
  );
  ...
  document.head.insertAdjacentHTML(
    "beforeend",
    `<style>select:focus{--tw-ring-shadow: none!important}</style>`,
  );
  ...
  editorCodeExample.innerHTML = `<code>main .h-full.flex-col > div
{
  ...
  saveChangesButton.setAttribute(
    "class",
    "btn flex justify-center gap-2 btn-primary mr-2",
  );
  ...
  cancelButton.setAttribute(
    "class",
    "btn flex justify-center gap-2 btn-neutral",
  );
...
function createMenuThemeEditor() {
  ...
  wrapper.setAttribute(
    "class",
    "flex flex-col items-center bg-gray-900 text-white space-y-1 p-2",
  );
